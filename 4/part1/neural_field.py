    ...
    for iteration in range(num_iterations):
        ...
        if save_progress and (iteration + 1) % 100 == 0:
            with torch.no_grad():
                ...
                cv2.imwrite(f"results/reconstruction_iter_{iteration+1}.png", reconstructed_image)
                model.train()
...
def reconstruct_image(model, dataset, output_path="reconstruction.png"):
    ...
    with torch.no_grad():
        ...
        reconstructed_image = (reconstructed_image * 255).astype(np.uint8)
        reconstructed_image = cv2.cvtColor(reconstructed_image, cv2.COLOR_RGB2BGR)
        cv2.imwrite(f"results/{output_path}", reconstructed_image)
        
    print(f"Reconstructed image saved to results/{output_path}")
